...
These devices tend to be simple and durable enought that they can be scavenged from old ruins, so even some smaller towns have may use a salvaged arcane heat system 

For more mundane heating methods, wood and charcoal burning is most used. Even wealthy individuals prefer cooking using mundaning heating methods due to the enhanced flavor.

## Communication

Magic: 
Paired scrying mirrors and sound crystals are most common. Multiply-linked exist, but are rarer and more expensive. Cheap ones may be always on, have poor range, and/or clarity.
Less common include linked journals (which or more of a luxury item, as they are expensive to manufacture, and run out of pages) or paired clay tablets.

Largest functioning linked broadcast system exists in the Empire of the Rising Sun.
Next biggest one is in 'jinn cities.

Direct telepathic communication may be used, but needs competent magic users who know each other on both sides, and is therefore only used between close friends and when extreme secrecy is needed.

Arcane constructs are sometimes used as couriers in areas with a particularily high concentration of mages, like the 'jinn principalities, hollow cities, or AoM&MA.

Mundane:
Foot couriers, or ships. Near free cities, 'fae couriers are prefered, for their superior speed.
Medium-short distances, or where cost is an issue.
Contracts, or other insances where a physical document is necessary.
Teleportation is sometimes used in such cases, but is not common. 
...
Methods include:
 - Using materials that disrupt or resist magic, such as silver or darkstone. 
 - Using materials that cannot be transmuted for other reasons, such as starfire, diamonds, and other crystals formed in high energy environments. Such currencies are most valuable among magic-users.
 - Using currencies that are magically valuable, such as arcana infused items or soul-infused items (immoral – mostly used by necromancers). Again, such currencies are most valuable among magic-users.
 ...
 - Paper money with various anti-magic, anti-counterfeiting options:
   - Silver content
   - Special alchemical compounds, including ones that ignite when touched by magic, or those that produce some other special effect.
   - Sympathetic signatures (often blood, or a material from a very specific location)
   - Magically sealed – unreliable, since the sealer must be a stronger mage than any potential counterfeiters.
 - Barter
...
Mages typically care more about sympathetic properties of materials, which means that classification of materials and compounds is pretty extensive, but few other developments come from that avenue.
...
Kinematics and Thermodynamics are exceptionally well understood, translating to both magical and mundane breakthroughs.


### Medicine

Anatomy, diagnosis, surgery, theory of disease are well developed.
Mundane treatment methods and pharmaceuticals are quite primitive, but even a novice hedgewitch can make fairly effective magical treatments.
